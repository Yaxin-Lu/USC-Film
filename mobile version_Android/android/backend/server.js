...
app.get("/", function (req, res) {
  res.send('hello, this is my backend of HW9 on gcp!');
});
    ...
    axios.get(url)
       .then((response) =>  {
         for(var i = 0; i<response.data.results.length; i++){
           ...
           tmp.imgs = 'https://image.tmdb.org/t/p/original' + element['backdrop_path'];
           ...
           tmp.vote = (parseFloat(element['vote_average'])/2).toFixed(1);
           //must check if date is exist??
           tmp.time = tmp.type=="movie"? element['release_date']:element['first_air_date'];
